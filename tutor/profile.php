...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['verification_pdf']) && $_FILES['verification_pdf']['error'] == UPLOAD_ERR_OK) {
   ...
   if ($pdf_uploaded) {
       $pdfUrl = "/vv/tutor/verification_pdfs/{$nic}_{$pdf['name']}";
        ...
        <div class="profile-card">
            ...
            <?php if ($profile && $profile['category'] === 'A/L'): ?>
                <p class="info-item"><span class="label">Subject Stream:</span> 
                    <?php echo $profile && !empty($profile['subject']) ? htmlspecialchars($profile['subject']) : 'Not entered'; ?>
                </p>
            ...
            <p class="info-item"><span class="label">Workplace:</span> <?php echo $profile && !empty($profile['workplace']) ? htmlspecialchars($profile['workplace']) : 'Not entered'; ?></p>
            <p class="info-item"><span class="label">Qualification:</span> <?php echo $profile && !empty($profile['qualification']) ? htmlspecialchars($profile['qualification']) : 'Not entered'; ?></p>
            <p class="info-item"><span class="label">About:</span> <?php echo $profile && !empty($profile['about']) ? htmlspecialchars($profile['about']) : 'Not entered'; ?></p>
        </div>
        <?php if ($verificationStatus !== 'approved'): // Check if status is not approved ?>
            <div class="verification-section">
                ...
                <form action="" method="post" enctype="multipart/form-data" class="upload-form" onsubmit="return confirmUpload();">
                    ...
                    <p class="verification-instruction">Make sure to upload a valid PDF of your NIC and Qualification for verification.</p>
                </form>
            </div>
        <?php endif; ?>
    </div>
...
<script>
    function confirmUpload() {
        return confirm("Are you sure you want to upload this PDF for verification?");
    }
</script>
</section>

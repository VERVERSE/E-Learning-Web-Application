...
if ($id) {
    $class_sql = mysqli_query($conn, "SELECT name, tutor_name, subject, time, date FROM courses WHERE id = '{$id}'");

    if (mysqli_num_rows($class_sql) > 0) {
        // Get details from the classes table
        $profile = mysqli_fetch_assoc($class_sql);
    } else {
        // No matching class or course found
        $profile = null;
    }
}
...
$query = "SELECT id, name, tutor_name, course, time, date FROM courses";

// If search query is present, add to the SQL query
if (!empty($search_query)) {
    $query .= " WHERE name LIKE '%{$search_query}%' OR tutor_name LIKE '%{$search_query}%' OR course LIKE '%{$search_query}%'";
}
...
// Deletion logic for classes or courses
if (isset($_GET['id'])) {
    // Sanitize the class ID
    $id = mysqli_real_escape_string($conn, $_GET['id']);

    // Check if the class exists
    $course_check = mysqli_query($conn, "SELECT * FROM courses WHERE id = '{$id}'");
...
<header class="header">
   <section class="flex">
      ...
      <form action="courses.php" method="GET" class="search-form">
         <input type="text" name="search_box" required placeholder="Search courses by name, tutor, or course..." maxlength="100" value="<?php echo htmlspecialchars($search_query); ?>">
               ...
               <tbody>
                  ...
                  if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                          $id = $row['id'];
                          $stream_or_course = !empty($row['course']) ? $row['course'] : 'N/A'; // Default to stream for classes
                          echo "<tr>
                              <td>{$row['name']}</td>
                              ...
                              <td>{$row['date']}</td>
                              <td>
                                  <a href='view_courses.php?nic={$id}' class='btn-view'>View</a>
                                  <a href='courses.php?id={$id}' class='btn-delete' onclick='return confirmDelete();'>Delete</a>
                              </td>
                          </tr>";
                      }
                  } else {

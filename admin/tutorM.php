...
if (isset($_GET['nic'])) {
    ...
    $nicToDelete = mysqli_real_escape_string($conn, $_GET['nic']);
    
    // Delete related records in the update_profile table
    mysqli_query($conn, "DELETE FROM update_profile WHERE nic = '{$nicToDelete}'");
    
    // Delete the record in the user_reg table
    $delete = mysqli_query($conn, "DELETE FROM user_reg WHERE nic = '{$nicToDelete}'");

    if ($delete) {
        // Redirect to the same page to reflect changes
        header("Location: tutorM.php");
        exit();
    } else {
...
$query = "SELECT name, nic, email FROM user_reg WHERE role = 'tutor'";
...
<header class="header">
   <section class="flex">
      ...
      <form action="tutorM.php" method="GET" class="search-form"> <!-- Changed action here -->
         <input type="text" name="search_nic" placeholder="Search by NIC or Name..." maxlength="50" value="<?php echo htmlspecialchars($search_query); ?>">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>
                            ...
                            <td>{$row['email']}</td>
                            <td>
                                <a href='view_tutor.php?nic={$nic}' class='btn-view'>View</a>
                                <a href='tutorM.php?nic={$nic}' class='btn-delete' onclick='return confirm(\"Are you sure you want to delete this tutor?\");'>Delete</a>
                            </td>
                        </tr>";
                    }
                } else {
